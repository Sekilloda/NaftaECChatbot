            ...
            if media_key and media_url:
                ...
                if encrypted_data:
                    ...
                    if decryption_successful:
                        print(f"Media decrypted and saved successfully: {output_path}") # Keep this print for now
                        if media_type == 'image':
                            print(f"Attempting OCR processing for image: {output_path}")
                            # output_filename is f'{message_id}{extension}' which is correct for original_filename
                            ocr_success = process_receipt_image(output_path, output_filename)
                            if ocr_success:
                                print(f"OCR processing successful for {output_filename}")
                                return jsonify({'status': 'media_and_ocr_processed_successfully', 'id': message_id, 'path': output_path, 'ocr_status': 'success'})
                            else:
                                print(f"OCR processing failed for {output_filename}")
                                return jsonify({'status': 'media_processed_ocr_failed', 'id': message_id, 'path': output_path, 'ocr_status': 'failed'})
                        else:
                            # For non-image media, no OCR
