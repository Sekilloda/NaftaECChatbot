...
SEQUENTIAL_PARAMS = [
    # Gemini
    {"model_name": "gemini-2.5-flash-lite", "context_window": 10, "temperature": 0.5},
    {"model_name": "gemini-2.5-flash-lite", "context_window": 10, "temperature": 1.5},
    {"model_name": "gemini-2.5-flash-lite", "context_window": 20, "temperature": 0.5},
    {"model_name": "gemini-2.5-flash-lite", "context_window": 20, "temperature": 1.5},
    # Mistral
    {"model_name": "mistral-small-3.2", "context_window": 10, "temperature": 0.15},
    {"model_name": "mistral-small-3.2", "context_window": 10, "temperature": 0.7},
    {"model_name": "mistral-small-3.2", "context_window": 20, "temperature": 0.15},
    {"model_name": "mistral-small-3.2", "context_window": 20, "temperature": 0.7},
]
...
""")
# --- Nueva tabla para log de parámetros por conversación ---
_cursor.execute("""
...
def get_conversation_params(user_id: str) -> dict:
    """
    Obtiene los parámetros de A/B testing para un usuario, gestionando la lógica de "conversaciones".
    Una nueva conversación se inicia si es la primera del usuario o si han pasado más de 60 minutos.
    """
    # 1. Modo Manual tiene prioridad
    if app.config.get('MANUAL_MODE'):
        manual_params = app.config.get('MANUAL_PARAMS', {})
        print(f"[A/B Test] Modo manual activo. Usando: {manual_params}")
        return manual_params

    user_id_norm = canonicalize_jid(user_id)
    
    # 2. Verificar si hay una conversación activa reciente (últimos 60 min)
    _cursor.execute(
        "SELECT model_name, context_window, temperature, created_at FROM conversation_log WHERE user_id=? ORDER BY created_at DESC LIMIT 1",
        (user_id_norm,)
    )
    last_log = _cursor.fetchone()

    if last_log:
        last_log_time = datetime.fromisoformat(last_log[3])
        if datetime.now() - last_log_time < timedelta(minutes=60):
            # Conversación activa, reutilizar parámetros
            return {"model_name": last_log[0], "context_window": last_log[1], "temperature": last_log[2]}

    # 3. Es una nueva conversación: asignar parámetros secuencialmente
    try:
        # Contar el total de conversaciones para determinar el índice
        _cursor.execute("SELECT COUNT(id) FROM conversation_log")
        total_conversations = _cursor.fetchone()[0]
        
        # Determinar el índice del conjunto de parámetros
        param_index = (total_conversations // 12) % len(SEQUENTIAL_PARAMS)
        new_params = SEQUENTIAL_PARAMS[param_index]

        # Guardar un nuevo registro para esta conversación
        _cursor.execute(
            "INSERT INTO conversation_log (user_id, model_name, context_window, temperature) VALUES (?, ?, ?, ?)",
            (user_id_norm, new_params["model_name"], new_params["context_window"], new_params["temperature"])
        )
        _conn.commit()
        print(f"[A/B Test] Nueva conversación para {user_id_norm}. Asignando set #{param_index}: {new_params}")
        return new_params

    except Exception as e:
        print(f"[DB] Error al crear log de conversación para {user_id_norm}: {e}")
        # Fallback a un default seguro si la DB falla
...
def responder(pregunta_actual, historial_conversacion="", k=2, model_name="gemini-2.5-flash-lite", temperature=1.0):
    """
...
def webhook():
    try:
        ...
        if pending_details:
            ...
            conv_params = get_conversation_params(sender)
...
if __name__ == "__main__":
    ...
    app.config['MANUAL_PARAMS'] = {}

    # Modo Manual: para forzar parámetros específicos desde la línea de comandos
    if '--manual' in sys.argv:
        ...
        manual_params = {}
        # Seleccionar modelo
        model_choice = input("Seleccione el modelo (1: Gemini, 2: Mistral): ")
        if model_choice == "1":
            manual_params["model_name"] = "gemini-2.5-flash-lite"
        else:
            manual_params["model_name"] = "mistral-small-3.2"

        # Ingresar context window
        try:
            manual_params["context_window"] = int(input("Ingrese el tamaño de la ventana de contexto (e.g., 10): "))
        except ValueError:
            print("Entrada inválida. Usando por defecto 10.")
            manual_params["context_window"] = 10
            
        # Ingresar temperatura
        try:
            manual_params["temperature"] = float(input("Ingrese la temperatura (e.g., 0.5): "))
        except ValueError:
            print("Entrada inválida. Usando por defecto 0.5.")
            manual_params["temperature"] = 0.5
        
        app.config['MANUAL_PARAMS'] = manual_params
        print(f"\nParámetros manuales configurados: {app.config['MANUAL_PARAMS']}")
        print("Iniciando servidor Flask con parámetros manuales...")

    # El modo de simulación no inicia el servidor, solo prepara el entorno.
    if '--simulate' in sys.argv:
        print("=== MODO SIMULACIÓN ACTIVADO ===")
        print("Servidor Flask no iniciado. Use un script de simulación para llamar a handle_webhook_data().")
    else:
        # Inicia el servidor en modo normal o manual, pero no en simulación.
